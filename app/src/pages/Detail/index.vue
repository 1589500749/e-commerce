...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      // 防止props传给zoom子组件的skuImageList是undefined,在获得服务器的数据之前至少传一个空数组给子组件
      skuImageList(){
    ...
    methods: {
      ...
      // 加入购物车
      async addShopcar(){
        // 请求成功则将产品加入数据库，服务器存储成功后进行路由跳转；若请求失败则给用户提示
        try {
          // 请求成功
          await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
          // 路由跳转
          // 一些简单的数据，例如skuNum这种通过query形式传给路由组件；复杂的数据例如skuInfo通过会话存储以字符串形式存入，路由组件AddCartSuccess通过会话存储取得。
